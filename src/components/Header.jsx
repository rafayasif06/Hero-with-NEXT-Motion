...
export const Header = ({ initialState, toggleMenuFunction }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY >= window.innerHeight * 0.6) {
        setIsVisible(true);
        setColor(initialState ? "#ffffff" : "#7c7262");
      } else {
        setIsVisible(false);
        setColor("#ffffff");
      }
  ...
  const toggleColor = () => {
    setColor((prevColor) => (prevColor === "#7c7262" ? "#FFFFFF" : "#7c7262"));
  };
  return (
    <header className=" z-[40] fixed top-0 left-0 flex   justify-between  items-center flex-row-reverse md:flex-row py-[8vw] md:py-[1.3rem] w-full px-[1.5rem]">
      ...
      >
        <div className="hamburger-menu-lines ">
          <motion.div
            animate={{
              backgroundColor: isVisible && !initialState ? "#7C7262" : "#FFFFFF",
            }}
        ...
        <button className="hidden md:block h-[1rem] overflow-hidden  cursor-pointer relative 2xl:h-[1vw]  ">
          <motion.h3
            animate={{
              color: isVisible && !initialState ? "#7C7262" : "#FFFFFF",
            }}
